    ...
    <div class="col-sm-9 col-8">
              ...
              <select name="state" id="state-dd" class="custom-select" autofocus>
                ...
                @foreach ($states as $state)
                  <option value="{{ $state->id }}"
                    {{ $state->id === old('state', $hostel->state) ? 'selected' : '' }}>{{ $state->name }}</option>
                @endforeach
              </select>
            </div>
          </div>
              ...
              <select id="period" name="period" class="custom-select custom-select-lg">
                ...
                @foreach ($periods as $period)
                  <option value="{{ $period->name }}"
                    {{ $period->name === old('period', $hostel->period) ? 'selected' : '' }}>{{ $period->name }}
                  </option>
                @endforeach
              </select>
            </div>
      ...
      <section id="photos" class="animated displayNone">
        ...
        <div class="form-group my-3">
          ...
          <p>Upload 1 clear image of the hostel for proper display. <br>jpg, jpeg & png format only.</p>
